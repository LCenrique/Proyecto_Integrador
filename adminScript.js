document.addEventListener("DOMContentLoaded", () => {
    ...
    // Botón de Cerrar Sesión (Mantiene LocalStorage intacto)
    ...
    const defaultCategories = [
        "cadenas", "anillos", "collares", "cinturones", "pulseras", 
        "botas", "playeras", "piercings", "accesorios", "herramientas", 
        "ropa", "calzado", "stickers"
    ];
        ...
        if (list) {
            list.innerHTML = categories.map((cat, idx) => `
                <li style="display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #2A2A2A;">
                    ...
                    <button class="btn-del" type="button" onclick="eliminarCategoria(${idx})">Eliminar</button>
                </li>
            `).join("");
        }
    ...
    window.eliminarAdmin = function(index) {
    ...
    /* ==========================================================================
       4. CARGA, TABLA Y PAGINACIÓN DE PRODUCTOS
       ========================================================================== */
    ...
    function renderAdminTable() {
        ...
        const statDestacados = document.getElementById("stat-destacados");

        if (statTotal) statTotal.textContent = products.length;
        if (statOfertas) statOfertas.textContent = products.filter(p => p.oferta).length;
        if (statDestacados) statDestacados.textContent = products.filter(p => p.destacado).length;
            ...
            <tr>
                ...
                <td>
                    <div class="admin-actions">
                        <button class="btn-edit" type="button" onclick="prepararEdicion(${p.id})">Editar</button>
                        <button class="btn-del" type="button" onclick="eliminarProducto(${p.id})">Eliminar</button>
                    </div>
                </td>
            </tr>
        `).join("");
    ...
    /* ==========================================================================
       5. EDICIÓN Y ELIMINACIÓN DE PRODUCTOS
       ========================================================================== */
    ...
    /* ==========================================================================
       6. CONTROL DEL MODAL Y DRAG & DROP
       ========================================================================== */
    ...
    if (productForm) {
        productForm.addEventListener("submit", (e) => {
            ...
            if (idVal !== "") {
                ...
                // Creación de producto con ID consecutivo automático
    ...
    /* ==========================================================================
       8. CONFIGURACIÓN DEL SISTEMA (WHATSAPP ATENCIÓN A CLIENTES)
       ========================================================================== */
    const DEFAULT_PHONE = "528991038598";
    const waInput = document.getElementById("store-whatsapp-input");
    const btnSaveWa = document.getElementById("btn-save-whatsapp");

    function obtenerNumeroWhatsApp() {
        return localStorage.getItem("streetSideWhatsapp") || DEFAULT_PHONE;
    }

    function cargarConfiguracionWhatsApp() {
        if (waInput) {
            const phone = obtenerNumeroWhatsApp();
            waInput.value = phone.startsWith("+") ? phone : `+${phone}`;
        }
    }

    window.guardarNuevoWhatsApp = function() {
        if (!waInput) return;

        let rawNumber = waInput.value.replace(/\D/g, "");
        ...
        localStorage.setItem("streetSideWhatsapp", rawNumber);
        showToast("Número de WhatsApp guardado con éxito");
        cargarConfiguracionWhatsApp();
    };

    if (btnSaveWa) {
        btnSaveWa.addEventListener("click", window.guardarNuevoWhatsApp);
    }

    /* ==========================================================================
       9. UTILIDADES E INICIALIZACIÓN
       ========================================================================== */
...
/* ==========================================================================
   Navegación Global entre pestañas (Llamada desde atributos onclick del HTML)
   ========================================================================== */
